...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer shippingId, Integer userId) {
       ...
       ServerResponse serverResponse= cartService.findCartByUserIdAndChecked(userId);
        ...
        for(Cart cart:cartList){
           OrderItem orderItem= cart2OrderItem(cart);
    ...
    public ServerResponse findOrderByOrderno(Long orderNo) {
        if (orderNo == null){
            return ServerResponse.createServerResponseByFail(ResponseCode.ORDER_NO_NOT_FAIL.getCode(),ResponseCode.ORDER_NO_NOT_FAIL.getMsg());
        }
    ...
    public ServerResponse updateOrderByPayed(Long orderNo, Integer status, Date payedTime) {
        /*if (orderNo == null){
            return ServerResponse.createServerResponseByFail(ResponseCode.ORDER_NO_NOT_FAIL.getCode(),ResponseCode.ORDER_NO_NOT_FAIL.getMsg());
        }*///这里不用写，因为再controller的支付成功后的逻辑中，一上来就判断是否有订单编号
        int count = orderMapper.updateOrderByPayed(orderNo,status,payedTime);
        if (count == 0){
            //更新失败
     ...
     * Cart转OrderItem
     * */
    private OrderItem cart2OrderItem(Cart cart){
